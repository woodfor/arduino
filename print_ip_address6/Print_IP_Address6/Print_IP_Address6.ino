...
bool setToken()
{
  ...
  {
    HTTPClient http; //Object of class HTTPClient
    String url = "http://capiwater-env.nnyv2mmdf9.ap-southeast-2.elasticbeanstalk.com/tap/token/" + tapID;
    http.begin(url);
    Serial.println(url);
    int httpCode = http.GET();
  ...
  { 
    ...
    if(flowRate > 0)
    {
      ...
      if(notification !=0)//open notification
      {
          if((sTimeCount > notification)&& (millis() - interval > 60000))
        {
          ...
          sendNotification(deviceName, tmp);
   ...
   else
    {
      ...
      { 
        String time = getTime();
        int count = 0;
        if(WiFi.status() == WL_CONNECTED && areaID !="0" && tapID !="0"){
            HTTPClient http;
            String url = "http://capiwater-env.nnyv2mmdf9.ap-southeast-2.elasticbeanstalk.com/report/"+areaID+"/"+
            tapID+"/"+time+"/"+String(totalMilliLitres,DEC)+"/"+ String(sTimeCount);
            http.begin(url);
            int httpResponseCode = http.PUT("");
            if(httpResponseCode>0){
                String response = http.getString();   
                Serial.println(response);          
                sendNotification(deviceName, "Data received");
                 }else{
